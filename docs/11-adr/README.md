...
| [ADR-008](#adr-008) | Backend Dil Seçimi: .NET 10 vs Java/Spring Boot | ✅ Kabul Edildi | 2026-05-20 |
...
### Backend Dil Seçimi: .NET 10 vs Java/Spring Boot

**Durum:** ✅ Kabul Edildi — 2026-05-20

#### Bağlam

Sistem tasarım dokümanı (readme.md) backend için "Java (Spring Boot) veya C# (.NET 8/Core)" seçeneği sunmaktaydı. ISO 8583 ekosisteminin olgun Java kütüphanesi (jPOS) nedeniyle Java ağırlıklı bir yaklaşım önerilmişti. Ancak ekibin mevcut yetenek seti ve altyapı gereksinimleri netleşince bir karar alınması gerekti.

#### Değerlendirilen Alternatifler

| Özellik | Java (Spring Boot) | C# (.NET 10) |
|---|---|---|
| **ISO 8583 Ekosistemi** | ✅ jPOS (olgun, kanıtlanmış) | ⚠️ OpenIso8583.Net veya custom parser |
| **Performans** | Yüksek (JVM JIT) | Yüksek (.NET Native AOT, daha az bellek) |
| **Ekip Yetkinliği** | Orta | ✅ Yüksek |
| **Mikroservis Olgunluğu** | ✅ Spring Cloud ekosistemi | ✅ ASP.NET Core, Dapr desteği |
| **SignalR / WebSocket** | ❌ Ek kütüphane gerekir | ✅ ASP.NET SignalR native |
| **Lisans Maliyeti** | Ücretsiz (OSS) | Ücretsiz (OSS) |
| **Docker İmaj Boyutu** | ~300 MB (JRE) | ~200 MB (ASP.NET runtime) |
| **Native AOT** | ❌ GraalVM (karmaşık) | ✅ .NET 10 native AOT |
...
- Ekibin mevcut yetkinliği .NET/C# üzerine yoğunlaşmıştır; öğrenme eğrisi ve geliştirme hızı kritik faktördür.
- ASP.NET SignalR, WebSocket bildirim altyapısı için native ve savaşta sınanmış bir çözüm sunar.
- .NET 10 ile gelen Native AOT ve minimal API özellikleri mikroservis container boyutunu önemli ölçüde azaltır.
- ISO 8583 kütüphanesi eksikliği PoC + custom parser ile giderilebilir (bkz. [ADR-004](#adr-004)); bu risk kabul edilebilir olarak değerlendirilmiştir.

#### Sonuçlar

- Tüm mikroservisler .NET 10 / ASP.NET Core ile geliştirilecek.
- ISO 8583 parser kararı ADR-004 kapsamında ayrıca netleştirilecek.
- jPOS referansları dokümanlarda "Java eşdeğeri" olarak kalabilir; .NET implementasyonunda karşılığı belirtilecek.
- Polly kütüphanesi (.NET) Retry/Circuit Breaker için standart olarak benimsenmiştir.
